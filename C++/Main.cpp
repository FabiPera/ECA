...
static void do_drawing(cairo_t *cr, ECA eca){
	...
	for(int i=0; i<eca.gens; i++){				
		for(int i=0; i<(eca.cells); i++){
			if(eca.t0[i]){
...
static void startSimulation(GtkWidget *btn, gpointer user_data){
	...
	GtkWidget *simWindow=gtk_application_window_new(app);
	gtk_window_set_title(GTK_WINDOW(simWindow), "Simulation");
	gtk_window_set_default_size(GTK_WINDOW(simWindow), (eca.cells*5), (400));
